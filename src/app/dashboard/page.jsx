...
export default function BarberDashboard() {
  ...
  // 🟢 The Ultimate NextAuth Init Effect
  useEffect(() => {
    if (status === "loading") return; 

    // Kick out unauthenticated users or users who aren't barbers
    if (status === "unauthenticated" || session?.user?.role !== "barber") {
    ...
    const fetchMyDashboard = async () => {
      try {
        ...
        const res = await fetch(`/api/barber/${myId}`);
        const data = await res.json();
        
        if (res.ok && data.data) {
          setBarber(data.data);
          setIsAvailable(data.data.isAvailable ?? true);
          if (data.data.nextAvailableAt) {
            setNextAvailableAt(toLocalISOString(data.data.nextAvailableAt));
          }
          // Fetch bookings immediately after getting the profile
          if(myId)fetchBookings(myId);
        }
      } catch (err) {
  ...
  const handleLogout = async () => {
    // NextAuth destroys the cookie safely. No localStorage needed!
    await signOut({ callbackUrl: "/barberLogin" }); 
  };

  // 🟢 Save Global Availability
  const saveAvailability = async () => {
    // Use the secure session ID
    const myId = session?.user?.id;
    if (!myId) return;
    ...
    try {
      ...
      // Update local React state (No localStorage!)
  ...
  const saveBlockedSlots = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/blockslots/${myId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          date: blockDate, 
  ...
  // Show the spinner if NextAuth is checking, OR if we are fetching data
            ...
            <div className="bg-slate-900 rounded-2xl shadow-xl border border-slate-800 p-6 sm:p-8">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 pb-4 border-b border-slate-800">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <button onClick={() => fetchBookings(session?.user?.id)} className="text-sm text-red-500 hover:text-red-400 font-medium transition-colors border border-red-500/30 px-3 py-2 rounded-lg bg-red-500/10">
                        Refresh
                      </button>
                    </div>
                </div>
