...
// Accept the server-fetched data as a prop
export default function HomeClient({ initialBarbers }) {
  ...
  // Initialize your state with the data passed from the server!
    ...
    try {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          setBarbers(data.data); // This safely overrides the initial server data!
          setIsLoading(false);
        },
        (error) => {
    ...
    try {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setMessages((prev) => [
            ...prev,
            { sender: "bot", text: "Unable to access your location." },
          ]);
          setIsLoading(false);
        }
      );
  ...
  return (
    <div className="relative min-h-screen bg-slate-950">
      <div className="p-8 flex flex-col gap-2 items-center">
        ...
        <button onClick={handleLocation} className="bg-red-500 hover:cursor-pointer p-2 rounded-3xl text-white font-semibold">
          Check Locality
        </button>
